from clases.claseClientes import Cliente
from clases.claseFacturas import Factura
from clases.claseProductos import Medicina
from clases.claseProductos import ControlDePlagas
from clases.claseProductos import ControlDeFertelizantes
...
def agregarProducto():
    ...
    if tipo == "M":
        ...
        producto = Medicina((len(listaProductos)+1), nombre, precio, dosis, tipoAnimal)
    ...
    if tipo == "P":
        ...
        if tipoProductoControl == "P":
            ...
            producto = ControlDePlagas((len(listaProductos)+1), nombre, precio, ica, frecuencia, periodoCarencia)
            listaProductos.append(producto)
        if tipoProductoControl == "F":
            ultimaAplicacion = input("Indique la última aplicación: ")
            producto = ControlDeFertelizantes((len(listaProductos)+1), nombre, precio, ica, frecuencia, ultimaAplicacion)
            listaProductos.append(producto)
